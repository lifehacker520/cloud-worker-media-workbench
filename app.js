...
function renderAccountHealth() {
  ...
  const rows = visibleAccounts.map((account) => {
    ...
    return (
      ...
      escapeHtml(account.state) +
      '\"><div class=\"monitor-account-main\">' +
      accountAvatarMarkup(account) +
      ...
      '</span>' +
      escapeHtml(subtitle) +
      '</small></div></div><div class=\"monitor-account-count\"><b>' +
      String(account.workCount || 0) +
      '</b><small>作品</small></div><div class=\"monitor-account-actions\"><span class=\"health-status ' +
      statusClass +
      '\">' +
      statusText +
      '</span><a href=\"' +
      escapeHtml(profileUrl) +
      '\" target=\"_blank\" rel=\"noreferrer\">打开 ↗</a></div></article>'
    );
  });
