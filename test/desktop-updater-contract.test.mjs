...
test('desktop updater bridge, release metadata, and one-click UI stay wired together', async () => {
  const [mainSource, preloadSource, appSource, indexSource, packageSource, afterSignSource] = await Promise.all([
  ...
  assert.match(afterSignSource, /designated => identifier/);
