# 云员工媒体工作台｜自媒体矩阵内容监控工作台

这是一个开源的 P0 工作台，用来把销售的小红书、抖音公开主页集中到一个界面里，定期读取公开主页，去重并展示新发现的作品。项目同时提供 Web 版和可下载的 Electron 桌面客户端，后续可以继续加入 AI 分析、提醒和更多平台。
...
- 从公开主页 HTML / 内嵌状态中提取作品标题、时间、点赞数、可用的作品链接和封面；作品流支持按平台筛选。
- 首次刷新建立基线；后续刷新对新指纹标记“新发现”。
- 监控中心支持按平台/状态筛选；删除账号前二次确认，并同步删除本地已抓取的该账号作品。
...
- 提供本地 API：/api/health、/api/auth/*、/api/state、/api/refresh、/api/accounts、/api/accounts/:id/seen、/api/works/seen、/api/feedback、/api/activity。

## 已知边界

这是可运行的单客户验证版，不是多租户生产 SaaS。小红书和抖音页面结构、反爬策略可能变化；部分主页虽然能返回作品标题，但没有稳定的作品 ID 或封面，这类作品会先链接回主页或显示封面占位。触发限流或安全校验时，账号会显示“需处理”，不会伪造成功结果。运行数据使用 JSON 文件，适合小规模试用，后续扩大客户数时应迁移到数据库并增加租户隔离。
...
抖音当前是 Beta 适配：分享短链会先解析到对应主页，但公开主页可能返回安全校验页，Demo 会明确记录失败原因，不绕过验证码、登录限制或平台安全机制。若要稳定监控自有抖音账号，下一步应接入抖音官方授权的数据能力，或增加需要用户主动登录的浏览器会话采集器；视频号、AI 摘要、钉钉/飞书通知、人工确认队列和通知 Webhook 仍未实现。
